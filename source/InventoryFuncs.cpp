...
static const UInt32 s_ExtraPoisonVtbl =		0x01079308;

ExtraPoison* ExtraPoisonCreate()
{
	ExtraPoison* xPoison = (ExtraPoison*)BSExtraData::Create(sizeof(ExtraPoison), s_ExtraPoisonVtbl);
	xPoison->poison = NULL;
	xPoison->unk08 = 0;
	return xPoison;
}
...
namespace InventoryFunctions_ReferenceUtils
{
	UInt32 SetPoison(TESForm * baseForm, BaseExtraList* extraData, AlchemyItem* poison, UInt32 value)
	{
		...
		newPoison->unk08 = value;
		extraData->Add(kExtraData_Poison, newPoison);

		//_MESSAGE("InventoryFunctions_ReferenceUtils::SetPoison - poisoned with %s, %d charges", newPoison->poison->fullName.GetName(), newPoison->unk08);
		return newPoison->unk08;
	}
...
namespace InventoryFunctions_Papyrus
{
	...
	template <class Op>
	UInt32 MyCountIfByKeyword(Op& op, TESObjectREFR* ObjRef, BGSKeyword* thisKeyword, bool Total)// const
	{
	...
	template <class Op>
	UInt32 MyCountIfByType(Op& op, TESObjectREFR* ObjRef, UInt32 type, bool Total)// const
	{
		TESContainer* Cont = NULL;
		Cont = DYNAMIC_CAST(ObjRef->baseForm, TESForm, TESContainer);
		UInt32 count = 0;
